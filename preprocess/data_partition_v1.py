...
def partition_data(fname_list, partition_by, data_path, ratio=[0.9, 0.05, 0.05]):
    arr_list = []
...
def main():
    ...
    parser.add_argument(
        '--option', default="none",
        choices=["cut_qu", "partition", "filter_train", "extract_feat", "none"])
    parser.add_argument(
        '--input_dir', '-i', default="", help="Data directory for original feature file")
    parser.add_argument(
        '--data_path', '-d', default="", help="Data directory for cut_qutime file")
    parser.add_argument(
        '--part_ratio', default="90,5,5", help="Partition ratio for train, validation, and test")
    parser.add_argument(
        '--rnd_ratio', default=0.1, type=float,
        help="For users with less history, only keep rnd_ratio of the data, can be 1")
    parser.add_argument(
        '--hist_len', default=5, type=int, help="Only keep users with query count >= hist_len")
    parser.add_argument(
        '--hist_len_ubound', default=11, type=int,
    ...
    if paras.option == "cut_qu":
        ...
        partition_data(fname_list, paras.partition_by, paras.data_path, ratio)
    elif paras.option == "filter_train":
        filter_train(paras.data_path, paras.hist_len, paras.hist_len_ubound, paras.rnd_ratio)
    elif paras.option == "extract_feat":
        fname_list = glob.glob("%s/*_data.gz" % (paras.input_dir))
        qset = collect_all_qset(paras.data_path, paras.hist_len)
        output_feat_file = "%s/extract_hl%d_feat_file.txt.gz" % (paras.data_path, paras.hist_len)
